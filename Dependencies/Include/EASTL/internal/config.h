...
// OCTENGINE LOCAL PATCH: upstream defaults this to EASTL_DEBUG, which puts a `const char* mpName` in
// every allocator - so every container is 8 bytes bigger in Debug than in the other configurations.
// Off in all of them. The prebuilt EASTL.lib / EASTLd.lib are built from these same headers, so the
// lib and the engine agree with no compiler flag. Re-apply when updating EASTL.
