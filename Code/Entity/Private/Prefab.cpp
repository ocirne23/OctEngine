...
static void writeOverrides(Entity* entity, AssetNode& node, const oc::string& tokenName)
{
    ...
    if (entity->getCullMode() == EEntityCullMode::RootOnly)
        node.set("CullMode", "RootOnly");
    else if (entity->getCullMode() == EEntityCullMode::None)
        node.set("CullMode", "None");
...
static void writeEntityBody(Entity* entity, AssetNode& node, const oc::string& tokenName)
{
    ...
    for (uint16 i = 0; i < MaxInlineComponentTypes; ++i)
    {
        ...
        else if (id == EComponentID_Particle)
        {
        ...
        else if (id == EComponentID_HumanoidAnimator)
        {
            if (const HumanoidAnimatorComponent::SpawnInfo* hi = getHumanoidAnimatorSpawnInfo(entity))
                writeHumanoidAnimatorSpawnInfo(*hi, comp);
        }
        else if (id == EComponentID_Network)
        {
            if (const NetworkComponent::SpawnInfo* ni = getNetworkSpawnInfo(entity))
                writeNetworkSpawnInfo(*ni, comp);
        }
