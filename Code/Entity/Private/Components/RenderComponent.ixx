...
export struct RenderComponent
{
    ...
    // Requires a valid node. The spatial entry refresh and the culled submit are the ENTITY's
    // (Entity::updateSelf): under CullMode RootOnly the pass mask comes from an ancestor.
    void place(const Transform& world);
};
