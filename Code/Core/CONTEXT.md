...
| `Core.BitPathTrie` | `oc::BitmapPathTrie` (builder) + `oc::BitmapPathTrieView` (lookup / iteration): a static, case-insensitive path trie with 16-byte nodes, laid out to bake to disk and memory-map. Not re-exported from `Core.ixx`. The format notes are at the top of the file. |
