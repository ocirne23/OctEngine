...
| 23 | terrain → collider → ocean → scatter → particles → force → **`renderer.kickLightGridBuild()`** | The force update is the frame's LAST light source, so the light grid's merge job kicks right after it (see RendererVK: the per-light work already ran inline in every `addLightInfo`). **Any new light source must be added before this kick.** |
