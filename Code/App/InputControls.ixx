...
public:
    ...
    void handleKeyEvent(const SDL_KeyboardEvent& evt)
    {
        ...
        if (evt.scancode == SDL_Scancode::SDL_SCANCODE_F7 && evt.type == SDL_EventType::SDL_EVENT_KEY_DOWN)
            Globals::profiler.writeReport(profileDumpPath, profileDumpOptions); // F7: text report of the last frames (same as --profile-after)
        ...
        if (evt.scancode == SDL_Scancode::SDL_SCANCODE_O && evt.type == SDL_EventType::SDL_EVENT_KEY_DOWN)
            renderer.cycleGiProbeDebugMode();        // O: cycle irradiance -> cellSize/LOD color -> update priority -> relocation / backface -> visibility
