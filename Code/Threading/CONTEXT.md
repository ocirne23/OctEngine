...
| `PerWorker<T>` | One cacheline-aligned `T` per **context**. `local()` / `at(i)` / `forEach`. Each slot is single-writer within a parallel phase; a downstream serial node drains them with `forEach`. **Re-read `local()` after any wait**, and pin a held `local()` reference with a `ThreadLocalScope`. Pure scratch that is never drained needs no PerWorker: a pinned `thread_local` does the same (the Nav seed-path A*). **Prefer OWNER-SLICED staging over PerWorker for a parallelFor's merge output**: `JobSystem::numChunks(count, grain)` slots indexed by `(chunkBegin - begin) / grain` — memory scales with the work, not the context count, and a wait inside the body cannot mix slots (the Force merge passes). |
