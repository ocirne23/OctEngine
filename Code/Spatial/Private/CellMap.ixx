...
struct CellRecord
{
    ...
    uint32 head = UINT32_MAX;  // first CellBlock of the cell's chain, UINT32_MAX = none
    uint32 count = 0;          // entries owning a lane here (blocks free as soon as they empty, so 0 <=> no chain)
};
