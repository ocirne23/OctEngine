...
// Sky for the film's mirror ray — mirrors ocean.fs.glsl's reflectedSkyRadiance (same step counts and
// eclipse saturation), so the film reflects the same sky the water next to it does. No sun disc: the
// GGX glint is its reflection.
